...
function createdObject(observableObject) {
        ...
        if (this.beforeEventStr != null) {
          ...
          if (propArray.length == 1) {
            ...
            if (this.lockKey == false) {
              // callfunc(this.valueBefore, changedvalue);
  ...
  this.on = function (eventStr, eventFunction) {
    ...
    callfunc = new Function(eventFunction);
  ...
  this.Next = function (eventStr, addEventListener) {
    ...
    this.addEventListener = function (valuesBefore, valuesAfter) {
      ...
      if (propArray.length == 1) {
        ...
        console.log('Value before prop change: ', valuesBefore);
        console.log('Value after prop change: ', valuesAfter);
        // addEventListener.call(valuesBefore, valuesAfter);
        return ('Value before prop change for ' + eventStr + ' is: ' + JSON.stringify(valuesBefore) + "|" + 'Value after prop change for ' + eventStr + 'is: ' + JSON.stringify(valuesAfter))
      }
      else {
        if (propArray.length == 2) {
...
app.get('/', function (req, res) {
  res.render('index', { title: " Welcome to State Liberary API'S ", result: '', response: 'No Output' });
});
...
app.post('/api/v1/state/create', function (req, res) {
  ...
  if (req.body == '' || JSON.parse(req.body.stateObject) == "" || JSON.parse(req.body.stateObject) == null) {
    res.render('index', { title: " Welcome to State Liberary API'S ", result: "State not Created! Please Pass Argument.", response: "No Output" })
  }
  else {
    let stateObj = JSON.parse(req.body.stateObject);
    if (typeof (stateObj) === 'object') {
      myState = create(stateObj);
      console.log(myState);
      res.render('index', { title: " Welcome to State Liberary API'S  ", result: "State Created!", response: myState })
        
    }
    else {
      res.render('index', { title: " Welcome to State Liberary API'S ", result: "State not Created! Please Pass object Argument.", response: "No Output " })
    }
  }
});
...
app.get('/api/v1/state/get', function (req, res) {
  ...
  res.render('index', { title: " Welcome to State Liberary API'S ", result: "State", response: response });
})

//API to Add Property
app.post('/api/v1/create/property', function (req, res) {
  console.log("body:", req.body);
  console.log("JSON object", JSON.parse(req.body.createPropObj));

  if (req.body == '' || JSON.parse(req.body.createPropObj) == "" || JSON.parse(req.body.createPropObj) == null) {
    res.render('index', { title: " Welcome to State Liberary API'S ", result: "State not Created! Please Pass Argument.", response: "No Output " })
  }
  else {
    var stateObj = JSON.parse(req.body.createPropObj);
    if (typeof (stateObj) === 'object') {
      console.log("My State: ", myState);
      myState.create(stateObj);
      res.render('index', { title: " Welcome to State Liberary API'S  ", result: "Property Added!", response: 'No Output ' })
    }
    else {
      res.render('index', { title: " Welcome to State Liberary API'S ", result: "State not Created! Please Pass object Argument.", response: "No Output " })
    }
...
app.post('/api/v1/get/property', function (req, res) {
  ...
  if (req.body == '' || req.body.getProperty == "" || req.body.getProperty == null) {
    ...
    if (response == undefined) {
      ...
      res.render('index', { title: " Welcome to State Liberary API'S  ", result: "Property Fetched!", response: response })
    }
  }
});
...
app.post('/api/v1/set/property', function (req, res) {
  ...
  if (req.body == '' || req.body.property == '' || req.body.propertyValue == '') {
    ...
    if (myState.beforeEventStr == null) {
      res.render('index', { title: " Welcome to State Liberary API'S  ", result: "Property Value Changed!", response: 'No Output ' })
    }
    else {
      if (response != undefined) {
